...
// Gap Feature: No webhooks for alert delivery
// No webhooks for alert delivery
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPrecisionFermentationProcessOptimizer / gap-no-webhooks-for-alert-delivery',
    },
    body: JSON.stringify({
...
router.post('/run', (req, res, next) => (aiRateLimiter ? aiRateLimiter(req, res, next) : next()), async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'No webhooks for alert delivery'.\nProject: AIPrecisionFermentationProcessOptimizer.\nContext: No webhooks for alert delivery\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['no-webhooks-for-alert-delivery', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'no-webhooks-for-alert-delivery', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
