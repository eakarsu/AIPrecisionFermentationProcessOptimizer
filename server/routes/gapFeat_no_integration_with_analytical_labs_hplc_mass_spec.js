...
// Gap Feature: No integration with analytical labs (HPLC, mass spec results)
// No integration with analytical labs (HPLC, mass spec results)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPrecisionFermentationProcessOptimizer / gap-no-integration-with-analytical-labs-hplc-mass-spec',
    },
    body: JSON.stringify({
...
router.post('/run', (req, res, next) => (aiRateLimiter ? aiRateLimiter(req, res, next) : next()), async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'No integration with analytical labs (HPLC, mass spec results)'.\nProject: AIPrecisionFermentationProcessOptimizer.\nContext: No integration with analytical labs (HPLC, mass spec results)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['no-integration-with-analytical-labs-hplc-mass-spec', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'no-integration-with-analytical-labs-hplc-mass-spec', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
...
router.get('/history', (req, res, next) => (aiRateLimiter ? aiRateLimiter(req, res, next) : next()), async (req, res) => {
  try {
    ...
    const r = await _pool.query(
      ...
      ['no-integration-with-analytical-labs-hplc-mass-spec']
    );
