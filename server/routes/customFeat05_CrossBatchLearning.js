...
// Aggregate data from hundreds of batches; identify key parameters driving yield/quality; auto-generate optimized recipes for new strains/targets
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetch('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPrecisionFermentationProcessOptimizer / cf-cross-batch-learning',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_cross_batch_learning`;
try {
  await pool.query(`
...
router.post('/run', aiRateLimiter, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant helping with the feature: 'Cross-batch learning'.
Context: Aggregate data from hundreds of batches; identify key parameters driving yield/quality; auto-generate optimized recipes for new strains/targets
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-cross-batch-learning/run error:', err);
