...
// Multiple ML models test feed compositions; ensemble recommends highest-yielding media; integrates cost optimization
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetch('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPrecisionFermentationProcessOptimizer / cf-media-optimization-ensemble',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_media_optimization_ensemble`;
try {
  await pool.query(`
...
router.post('/run', aiRateLimiter, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant helping with the feature: 'Media optimization ensemble'.
Context: Multiple ML models test feed compositions; ensemble recommends highest-yielding media; integrates cost optimization
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-media-optimization-ensemble/run error:', err);
