...
// Multi-modal sensor fusion (turbidity, dissolved O2, metabolite profiles); AI predicts contamination 12-24 hours before it becomes obvious; suggests remediation actions
...
// OpenRouter helper — inline to keep this scaffold self-contained
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetch('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPrecisionFermentationProcessOptimizer / cf-contamination-risk-early-warning',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_contamination_risk_early_warning`;
try {
  await pool.query(`
...
router.post('/run', aiRateLimiter, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant helping with the feature: 'Contamination risk early warning'.
Context: Multi-modal sensor fusion (turbidity, dissolved O2, metabolite profiles); AI predicts contamination 12-24 hours before it becomes obvious; suggests remediation actions
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-contamination-risk-early-warning/run error:', err);
