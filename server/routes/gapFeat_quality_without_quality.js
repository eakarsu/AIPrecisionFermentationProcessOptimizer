...
// Gap Feature: Quality without \'/quality
// Quality without /quality-anomaly-detect (flag out-of-spec results)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPrecisionFermentationProcessOptimizer / gap-quality-without-quality',
    },
    body: JSON.stringify({
...
router.post('/run', (req, res, next) => (aiRateLimiter ? aiRateLimiter(req, res, next) : next()), async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'Quality without \'/quality'.\nProject: AIPrecisionFermentationProcessOptimizer.\nContext: Quality without /quality-anomaly-detect (flag out-of-spec results)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['quality-without-quality', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'quality-without-quality', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
