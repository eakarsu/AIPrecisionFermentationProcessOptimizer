...
// Gap Feature: Limited regulatory documentation (cGMP, FDA compliance)
// Limited regulatory documentation (cGMP, FDA compliance)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPrecisionFermentationProcessOptimizer / gap-limited-regulatory-documentation-cgmp-fda-complian',
    },
    body: JSON.stringify({
...
router.post('/run', (req, res, next) => (aiRateLimiter ? aiRateLimiter(req, res, next) : next()), async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'Limited regulatory documentation (cGMP, FDA compliance)'.\nProject: AIPrecisionFermentationProcessOptimizer.\nContext: Limited regulatory documentation (cGMP, FDA compliance)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['limited-regulatory-documentation-cgmp-fda-complian', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'limited-regulatory-documentation-cgmp-fda-complian', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
...
router.get('/history', (req, res, next) => (aiRateLimiter ? aiRateLimiter(req, res, next) : next()), async (req, res) => {
  try {
    ...
    const r = await _pool.query(
      ...
      ['limited-regulatory-documentation-cgmp-fda-complian']
    );
