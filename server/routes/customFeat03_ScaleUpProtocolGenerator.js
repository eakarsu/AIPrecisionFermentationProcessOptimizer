...
// Lab-scale parameters input; AI generates scale-up recipe for production bioreactors; accounts for heat transfer, mixing differences; predicts production yields
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetch('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPrecisionFermentationProcessOptimizer / cf-scale-up-protocol-generator',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_scale_up_protocol_generator`;
try {
  await pool.query(`
...
router.post('/run', aiRateLimiter, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant helping with the feature: 'Scale-up protocol generator'.
Context: Lab-scale parameters input; AI generates scale-up recipe for production bioreactors; accounts for heat transfer, mixing differences; predicts production yields
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-scale-up-protocol-generator/run error:', err);
