...
// Gap Feature: Costs without \'/cost
// Costs without /cost-optimization (media optimization, energy efficiency)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPrecisionFermentationProcessOptimizer / gap-costs-without-cost',
    },
    body: JSON.stringify({
...
router.post('/run', (req, res, next) => (aiRateLimiter ? aiRateLimiter(req, res, next) : next()), async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'Costs without \'/cost'.\nProject: AIPrecisionFermentationProcessOptimizer.\nContext: Costs without /cost-optimization (media optimization, energy efficiency)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['costs-without-cost', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'costs-without-cost', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
