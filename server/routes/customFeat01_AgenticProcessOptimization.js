...
// Real-time bioreactor data streams; AI continuously adjusts parameters (feed rate, aeration, temperature) to maximize yield while maintaining quality; learns from batch outcomes
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetch('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPrecisionFermentationProcessOptimizer / cf-agentic-process-optimization',
    },
    body: JSON.stringify({
...
const TABLE_NAME = `custom_feat_cf_agentic_process_optimization`;
try {
  await pool.query(`
...
router.post('/run', aiRateLimiter, async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant helping with the feature: 'Agentic process optimization'.
Context: Real-time bioreactor data streams; AI continuously adjusts parameters (feed rate, aeration, temperature) to maximize yield while maintaining quality; learns from batch outcomes
...
If the user payload is insufficient, populate fields with reasonable defaults and note assumptions in summary.`;
    ...
    console.error('cf-agentic-process-optimization/run error:', err);
