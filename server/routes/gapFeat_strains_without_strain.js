...
// Gap Feature: Strains without \'/strain
// Strains without /strain-performance-predict (growth rate, yield, stress response)
...
async function callLLM(systemPrompt, userPrompt) {
  ...
  const res = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIPrecisionFermentationProcessOptimizer / gap-strains-without-strain',
    },
    body: JSON.stringify({
...
router.post('/run', (req, res, next) => (aiRateLimiter ? aiRateLimiter(req, res, next) : next()), async (req, res) => {
  try {
    ...
    const systemPrompt = `You are an expert assistant for the gap feature: 'Strains without \'/strain'.\nProject: AIPrecisionFermentationProcessOptimizer.\nContext: Strains without /strain-performance-predict (growth rate, yield, stress response)\nReturn structured JSON with keys: { summary: string, recommendations: string[], risks: string[], next_steps: string[] }.`;
    ...
    if (_pool && _tableReady) {
      try {
        const r = await _pool.query(
          ...
          ['strains-without-strain', req.user?.id || null, JSON.stringify(payload), out.content, out.tokensUsed, out.model]
        );
    ...
    res.json({ ok: true, id: savedId, feature: 'strains-without-strain', result: out.content, tokens_used: out.tokensUsed, model: out.model });
  } catch (e) {
