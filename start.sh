...
(cd "$PROJECT_DIR/server" && SERVER_PORT="$BACKEND_PORT" node index.js) &
CHILD_PIDS+=("$!")
(cd "$PROJECT_DIR/client" && npm run dev -- --port "$FRONTEND_PORT" --host 127.0.0.1) &
CHILD_PIDS+=("$!")

echo "Fermentation services started without installing, seeding, migrating, or reclaiming ports."
wait "${CHILD_PIDS[@]}"
